...
export default function Login() {
  ...
  const [email, setEmail] = useState(ADMIN_DEFAULT_CREDENTIALS.email);
  const [password, setPassword] = useState(ADMIN_DEFAULT_CREDENTIALS.password);
    ...
    <main className="min-h-screen bg-background bg-mesh flex items-center justify-center px-4 py-12">
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-muted-foreground mt-1">{SITE.brand} portfolio control panel</p>
        </div>
        ...
        <div className="mt-6 rounded-lg border border-border bg-secondary/40 p-3 text-xs text-muted-foreground">
          ...
          <p>Email: {ADMIN_DEFAULT_CREDENTIALS.email}</p>
          <p>Password: {ADMIN_DEFAULT_CREDENTIALS.password}</p>
        </div>
      </motion.div>
    </main>
  );
